...
(() => {
  ...
  function hubNameFromPath(pathname) {
    let p = pathname || "";
    try {
  ...
  function rootNameForDocument(doc, loc) {
    ...
    return hubNameFromPath(bootPath);
  }
  ...
  function boot(runtime, doc = document) {
    const parsed = parseHubDocument(doc);
    ...
    fetch(location.href).then((res) => res.ok ? res.text() : "").then((t) => {
      const raw = t ? parseHubText(t) : null;
